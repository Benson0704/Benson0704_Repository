...
while 1:
    ...
    run(sys.argv[1], stdin=open("input.txt"), stdout=open("arg1_output.txt", "w"),
        timeout=5., shell=True).check_returncode()
    run(sys.argv[2], stdin=open("input.txt"), stdout=open("arg2_output.txt", "w"),
        timeout=5., shell=True).check_returncode()
    if not check_ans("arg1_output.txt", "arg2_output.txt"):
        print("Wrong")
        break
    print(' ')
    print("OK")
